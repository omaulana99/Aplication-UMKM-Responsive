...
// Event listener untuk membuka keranjang
cartIcon.addEventListener("click", () => {
    const count = parseInt(itemCount.textContent); // Ambil jumlah item di keranjang

    // Periksa apakah keranjang kosong
    if (count === 0) {
        showNotification("Silakan lakukan pemesanan terlebih dahulu.");
    } else {
        cart.classList.add("active"); // Tampilkan keranjang jika ada item
    }
});

// Event listener untuk menutup keranjang
closeCart.addEventListener("click", () => {
    cart.classList.remove("active"); // Sembunyikan keranjang
});
...
// Add an event listener to toggle the menu when the hamburger icon is clicked
